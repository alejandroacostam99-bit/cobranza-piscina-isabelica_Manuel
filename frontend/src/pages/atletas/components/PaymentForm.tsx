...
export const PaymentForm: React.FC<PaymentProps> = ({ paymentData, setPaymentData }) => {
  ...
  // Efecto de Cálculo de Fecha
  useEffect(() => {
    if (paymentData.paymentDate) {
      const [yearStr, monthStr, dayStr] = paymentData.paymentDate.split('-');
      ...
      if (paymentData.coverageDate !== nextMonthStr) {
          setPaymentData(prev => ({ ...prev, coverageDate: nextMonthStr }));
      }
    }
  }, [paymentData.paymentDate, setPaymentData]); 
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
             ...
             <div>
               ...
               <input 
                 ...
                 onChange={handleChange}
                 className="w-full bg-green-50 border border-green-200 rounded-lg p-2.5 text-sm text-green-700 font-bold focus:outline-none focus:border-green-500 focus:ring-2 focus:ring-green-500/20" 
               />
             </div>
           </div>
        </div>
