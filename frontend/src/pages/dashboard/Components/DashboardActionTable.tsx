...
import { obtenerAtencionesDeDashboard } from '../Services/dashboard.service';
...
export const DashboardActionTable = () => {
    ...
    let datosProcesados = datosReales.map(item => {
        ...
        if (diasRestantes < 0) {
            estadoTexto = `Venció hace ${Math.abs(diasRestantes)} día(s)`;
            badgeColor = 'bg-rose-50/80 text-rose-700 border-rose-100 backdrop-blur-sm';
        } else if (diasRestantes === 0) {
            estadoTexto = 'Vence HOY';
            badgeColor = 'bg-amber-50/80 text-amber-700 border-amber-100 backdrop-blur-sm';
        } else {
        ...
        return { ...item, diasRestantes, estadoTexto, badgeColor, iniciales, nombreAtleta };
    });
        ...
        <div className="w-full mt-10 mb-10">
            <div className="flex justify-between items-end mb-6 px-2">
                <div>
                    ...
                    <p className="text-sm lg:text-base text-slate-500 ml-5">Vencimientos y deudas urgentes.</p>
                </div>
                <button
                    ...
                    <table className="w-full text-left border-collapse min-w-[500px]">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100 text-slate-400 text-xs lg:text-sm uppercase tracking-wider">
                                ...
                                <th className="p-5 lg:p-6 font-bold text-right">Acción</th>
                            </tr>
                        </thead>
                                ...
                                <tr key={item.id} className="hover:bg-slate-50/50 transition-colors group">
                                    <td className="p-4 lg:p-6">
                                        <div className="flex items-center gap-4">
                                            <div className="w-10 h-10 lg:w-12 lg:h-12 rounded-xl lg:rounded-2xl bg-slate-50 border border-slate-100 text-slate-600 flex items-center justify-center font-black text-sm lg:text-base shadow-sm group-hover:bg-white transition-colors">
                                                {item.iniciales}
                                            </div>
                                            <span className="font-bold text-slate-700 lg:text-lg group-hover:text-slate-900 transition-colors">{item.nombreAtleta}</span>
                                        </div>
                                    </td>

                                    <td className="p-4 lg:p-6">
                                        <span className={`px-3 py-1.5 rounded-xl text-xs font-black border shadow-sm ${item.badgeColor}`}>
                                            {item.estadoTexto}
                                        </span>
                                    </td>
